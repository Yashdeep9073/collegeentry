<script setup>
import { ref } from "vue";
// Note: We are keeping the Heroicons for the password toggle, 
// as the target CSS does not include a specific eye icon.
...
// Function to switch to register, wrapped for clarity
...
<template>
  ...
  >
    <div class="form_container relative animate-[fadeIn_.3s]">
      <button
        @click="$emit('close')"
      ...
      <div class="title_container">
        ...
        <span class="subtitle">Get started with our app, just log in and enjoy the experience.</span>
      </div>
...
<style scoped>
/* Removed @reference "tailwindcss"; and the original .input definition.
  The original styles are pasted below. 
  Note: Tailwind's `animate-[fadeIn_.3s]` is kept in the template for the fade animation.
*/

/* --- PASTED STYLES FOR NEW DESIGN --- */
.form_container {
    width: fit-content;
    height: fit-content;
    display: flex;
  ...
  .input_field {
    width: 100%; /* Changed from auto to 100% for proper modal width */
    height: 40px;
    padding: 0 0 0 40px;
    border-radius: 7px;
    outline: none;
  ...
  .sign-in_btn {
    ...
    height: 40px;
    border: 0;
    /* Changed background from #115DFC to your brand orange */
    background: #FF5C00; 
    border-radius: 7px;
    outline: none;
    color: #ffffff;
  ...
  /* Retained for completeness, though buttons are commented out */
